...
class SuraLangController extends Controller
{
    ...
    {
        return DB::select(
                'select sura_langs.sura, sura_langs.iso_code as lang, sura_langs.name, sura_langs.location, COUNT(suras.aya) as ayahs, sura_reciters.audio
                        from sura_langs
                ...
                [$request->lang, $request->reciter_id]);
    }

    public function show(Request $request, $sura)
    {
        return DB::select(
            'select sura_langs.id, sura_langs.aya, sura_langs.text, sura_langs.name, suras.text as original, sura_reciters.audio
                    from sura_langs
                     join sura_reciters on sura_langs.sura = sura_reciters.sura_id
                      join suras on sura_langs.sura = suras.sura and sura_langs.aya = suras.aya where sura_langs.sura = ? and sura_langs.iso_code = ?',
            [$sura, $request->lang, $request->reciter_id]);
    }
